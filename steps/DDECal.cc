...
DDECal::DDECal(const common::ParameterSet& parset, const std::string& prefix)
    : settings_(parset, prefix),
      average_time_(0),
      solutions_(),
      initial_solutions_(parset, prefix),
...
void DDECal::updateInfo(const DPInfo& infoIn) {
  ...
  initial_solutions_.Initialize(patches_per_direction_.size(),
...
void DDECal::InitializeSolutions(size_t buffer_index) {
  ...
  const bool use_initial_solutions = !initial_solutions_.Empty();
  ...
  if (propagate_solutions) {
    ...
    initial_solutions_.Read(settings_, solutions_[solution_index],
                            getInfoOut().GetUsedAntennaNames(),
                            channel_block_frequencies_, source_directions_,
                            average_time_);
  } else {
