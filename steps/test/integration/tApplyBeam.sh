...
INIT=testInit.sh
if [ ! -f $INIT ]; then
  echo $INIT not found. Please run this script from build/steps/test.
  exit 1;
fi
source $INIT
...
cmd="$dp3exe msin=tNDPPP-generic.MS msout=outinv.ms steps=[applybeam] applybeam.usechannelfreq=false applybeam.invert=true"
echo $cmd
$cmd
...
cmd="$dp3exe msin=outinv.ms msout=out.ms steps=[applybeam] applybeam.usechannelfreq=false applybeam.invert=false"
echo $cmd
$cmd
...
cmd="$dp3exe msin=tNDPPP-generic.MS msout=outinv.ms msout.overwrite=true steps=[applybeam] applybeam.usechannelfreq=true applybeam.invert=true"
echo $cmd
$cmd
# Compare the DATA column of the output MS with the BBS reference output.
...
echo; echo "Test with invert=false on the output of the previous step"; echo
cmd="$dp3exe msin=outinv.ms msout=out.ms msout.overwrite=true steps=[applybeam] applybeam.usechannelfreq=true applybeam.invert=false"
echo $cmd
$cmd
# Compare the DATA column of the output MS with the original MS.
taqlcmd='select from out.ms t1, tNDPPP-generic.MS t2 where not all(near(t1.DATA,t2.DATA,8e-5) || (isnan(t1.DATA) && isnan(t2.DATA)))'
...
cmd="$dp3exe msin=tNDPPP-generic.MS msout=outinv.ms msout.overwrite=true steps=[applybeam] applybeam.usechannelfreq=true applybeam.invert=true applybeam.beammode=ARRAY_FACTOR"
echo $cmd
$cmd
# Compare the DATA column of the output MS with the BBS reference output.
...
cmd="$dp3exe msin=tNDPPP-generic.MS msout=outinv.ms msout.overwrite=true steps=[applybeam] applybeam.usechannelfreq=true applybeam.invert=true applybeam.beammode=ELEMENT"
echo $cmd
$cmd
# Compare the DATA column of the output MS with the BBS reference output.
...
cmd="$dp3exe msin=tNDPPP-generic.MS msout=. steps=[applybeam] applybeam.updateweights=truue msout.weightcolumn=NEW_WEIGHT_SPECTRUM"
echo $cmd
$cmd
