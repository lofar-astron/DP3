...
const std::string& BlobIStream::getNextType (uint64_t& size)
{
  ...
  if (itsLevel == 0) {
    ...
    itsDataFormat  = DP3::DataFormat(hdr.itsDataFormat);
  } else {
...
void BlobIStream::getBuf (void* buf, uint64_t sz)
{
  ...
  if (sz1 != sz) {
      throw BlobException(
	     "BlobIStream::getBuf - " + std::to_string(sz) + " bytes asked, but only "
	     + std::to_string(sz1) + " could be read (pos=" + std::to_string(tellPos()) + ")");
  }
  itsCurLength += sz1;
}
...
BlobIStream& BlobIStream::operator>> (int16_t& var)
{
  ...
  if (itsMustConvert) {
    var = DP3::dataConvert (itsDataFormat, var);
  }
  return *this;
}
...
BlobIStream& BlobIStream::operator>> (float& var)
{
  getBuf (&var, sizeof(var));
  if (itsMustConvert) {
    DP3::dataConvertFloat (itsDataFormat, &var);
  }
  return *this;
}
BlobIStream& BlobIStream::operator>> (double& var)
{
  getBuf (&var, sizeof(var));
  if (itsMustConvert) {
    DP3::dataConvertDouble (itsDataFormat, &var);
  }
  return *this;
}
BlobIStream& BlobIStream::operator>> (std::complex<float>& var)
{
  getBuf (&var, sizeof(var));
  if (itsMustConvert) {
    DP3::dataConvertFloat (itsDataFormat, &var, 2);
  }
  return *this;
}
BlobIStream& BlobIStream::operator>> (std::complex<double>& var)
{
  getBuf (&var, sizeof(var));
  if (itsMustConvert) {
    DP3::dataConvertDouble (itsDataFormat, &var, 2);
  }
  return *this;
}
...
void BlobIStream::get (bool* values, uint64_t nrval)
{
  ...
  while (nrval > 0) {
    ...
    int nrb = (nr+7)/8;
    getBuf (buf, nrb);
    DP3::bitToBool (values, buf, nr);
    nrval -= nr;
    values += nr;
  }
}
void BlobIStream::get (char* values, uint64_t nrval)
{
  getBuf (values, nrval);
}
void BlobIStream::get (int8_t* values, uint64_t nrval)
{
  getBuf (values, nrval);
}
...
void BlobIStream::get (int16_t* values, uint64_t nrval)
{
  ...
  if (itsMustConvert) {
    DP3::dataConvert16 (itsDataFormat, values, nrval);
  }
}
...
void BlobIStream::get (int32_t* values, uint64_t nrval)
{
  ...
  if (itsMustConvert) {
    DP3::dataConvert32 (itsDataFormat, values, nrval);
  }
}
...
void BlobIStream::get (int64_t* values, uint64_t nrval)
{
  ...
  if (itsMustConvert) {
    DP3::dataConvert64 (itsDataFormat, values, nrval);
  }
}
...
void BlobIStream::get (float* values, uint64_t nrval)
{
  ...
  if (itsMustConvert) {
    DP3::dataConvertFloat (itsDataFormat, values, nrval);
  }
}
void BlobIStream::get (double* values, uint64_t nrval)
{
  ...
  if (itsMustConvert) {
    DP3::dataConvertDouble (itsDataFormat, values, nrval);
  }
}
void BlobIStream::get (std::complex<float>* values, uint64_t nrval)
{
  ...
  if (itsMustConvert) {
    DP3::dataConvertFloat (itsDataFormat, values, 2*nrval);
  }
}
void BlobIStream::get (std::complex<double>* values, uint64_t nrval)
{
  ...
  if (itsMustConvert) {
    DP3::dataConvertDouble (itsDataFormat, values, 2*nrval);
  }
}
