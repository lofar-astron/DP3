...
class BlobHeader {
 ...
 public:
  ...
  int getVersion() const {
    return (mustConvert() ? DP3::dataConvert(getDataFormat(), itsVersion)
                          : itsVersion);
  }

  /// Get the length of the blob. Data will be converted if needed.
  uint64_t getLength() const {
    return (mustConvert() ? DP3::dataConvert(getDataFormat(), itsLength)
                          : itsLength);
  }
  ...
  bool mustConvert() const { return itsDataFormat != DP3::dataFormat(); }
