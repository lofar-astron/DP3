...
template <typename T> void register_cube(py::module &m, const char *name)
{
    ...
    .def_buffer([](casacore::Cube<T> &cube) -> py::buffer_info {
            return py::buffer_info(
                ...
                { cube.shape()[2], cube.shape()[1], cube.shape()[0] },                 /* Buffer dimensions */
                { sizeof(T) * cube.shape()[1] * cube.shape()[0],             /* Strides (in bytes) for each index */
                    sizeof(T) * cube.shape()[0],
                    sizeof(T) }
            );
        });
}

template <typename T> void register_matrix(py::module &m, const char *name)
{
    ...
    .def_buffer([](casacore::Matrix<T> &matrix) -> py::buffer_info {
            return py::buffer_info(
                ...
                { sizeof(T) * matrix.shape()[0],              /* Strides (in bytes) for each index */
                  sizeof(T) }
            );
        });
}
...
PYBIND11_MODULE(pydppp, m) {
    ...
    > (m, "DPStep")
        ...
        .def("get_next_step", &DPStepWrapper::get_next_step, py::return_value_policy::copy)
    ...
    py::class_<DPBuffer, std::shared_ptr<DPBuffer>> (m, "DPBuffer")
        .def(py::init<>())
        .def("get_data", (casacore::Cube<casacore::Complex>& (DPBuffer::*)()) (&DPBuffer::getData))
        .def("get_weights", (casacore::Cube<float>& (DPBuffer::*)()) (&DPBuffer::getWeights))
        .def("get_flags", (casacore::Cube<bool>& (DPBuffer::*)()) (&DPBuffer::getFlags))
