...
PythonStep::PythonStep(DPInput* input, const ParameterSet& parset,
                       const string& prefix)
    : itsInput(input),
      itsName(prefix),
  ...
  try {
    ...
    boost::python::object dpppModule =
        boost::python::import(itsPythonModule.c_str());
    // Get the python class object from the imported module
    boost::python::object dpppAttr = dpppModule.attr(itsPythonClass.c_str());
...
bool PythonStep::process(const DPBuffer& buf) {
  try {
    ...
    boost::python::object result =
        itsPyObject.attr("process")(itsBufIn.getTime(), itsBufIn.getExposure());
...
void PythonStep::setNeedVisData() { info().setNeedVisData(); }
void PythonStep::setNeedWrite() { info().setWriteData(); }
