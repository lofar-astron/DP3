...
/// @{
/// A separate data stream is used for each direction. The mixing coefficients
/// quantify the influence of each direction on each of the other directions
/// (including time and frequency smearing).
class EstimateNew {
 public:
  EstimateNew();
  ...
  const std::vector<double>& getSolution() const { return itsSolution; }

  /// Get the nr of iterations used.
  size_t nIterations() const { return itsNrIter; }
 ...
 private:
  ...
  std::vector<dcomplex> itsdM;
  std::vector<double> itsdR;
