...
DemixWorker::DemixWorker(const std::string& prefix, const DemixInfo& mixInfo,
  ...
  itsPredictVis.resize(itsMix->ncorr(), itsMix->nchanOut(), itsMix->nbl());
  ...
  for (unsigned int dr = 0; dr < nsrc + 1; ++dr) {
    itsModelVisDemix[dr].resize(itsMix->ncorr(), itsMix->nchanOut(),
                                itsMix->nbl());
  }
  itsModelVisSubtr.resize(nsrc + 1);
  for (unsigned int dr = 0; dr < nsrc + 1; ++dr) {
    itsModelVisSubtr[dr].resize(itsMix->ncorr(), itsMix->nchanOutSubtr(),
                                itsMix->nbl());
  }
  ...
  itsTargetAmpl.resize(itsMix->nchanOut(), itsMix->nbl(), itsMix->ntimeOut());
...
void DemixWorker::process(const DPBuffer* bufin, unsigned int nbufin,
  ...
  if (itsMix->verbose() > 10) {
    std::cout << "\nInfo for time chunk " << chunkNr << " ( " << nbufin
              << " time slots)\n"
              << "===================\n";
  }
  ...
  unsigned int ntime =
      avgSplitUVW(bufin, nbufin, itsMix->ntimeAvg(), itsFilter.getIndicesBL());
  ...
  for (unsigned int i = 0; i < nbufin; ++i) {
    ...
    if ((i + 1) % itsMix->ntimeAvg() == 0 || i == nbufin - 1) {
      makeFactors(
          itsFactorBuf, itsFactors[itsNTimeOut],
          itsAvgResults[itsSrcSet[0]]->get()[itsNTimeOut].GetCasacoreWeights(),
          itsMix->nchanOut(), itsMix->nchanAvg());
    ...
    if ((i + 1) % itsMix->ntimeAvgSubtr() == 0 || i == nbufin - 1) {
      makeFactors(
          ...
          itsMix->nchanOutSubtr(), itsMix->nchanAvgSubtr());
...
unsigned int DemixWorker::avgSplitUVW(const DPBuffer* bufin,
                                      unsigned int nbufin,
  ...
  for (unsigned int time = 0; time < ntime; ++time) {
    ...
    if (itsMix->verbose() > 12) {
      std::cout << "avguvw=" << ntodo << itsAvgUVW << '\n';
    }
    nleft -= ntodo;
...
void DemixWorker::predictAteam(
  ...
  itsSrcSet.resize(0);
  std::vector<unsigned int> antCount(itsMix->nstation());
  for (unsigned int i = 0; i < patchList.size(); ++i) {
    ...
    for (unsigned int j = 0; j < itsMix->nbl(); ++j, miter.next()) {
      if (itsMix->verbose() > 11) {
        std::cout << "source " << i << ", baseline " << itsMix->getAnt1()[j]
                  << ' ' << itsMix->getAnt2()[j] << " has max ampl "
                  << max(miter.matrix()) << '\n';
      }
    ...
    for (unsigned int j = 0; j < antCount.size(); ++j) {
      if (antCount[j] >= itsMix->minNBaseline()) {
        ...
        if (itsMix->verbose() > 10) {
          std::cout << "ignore station " << j << " for source "
                    << patchList[i]->name() << "  (occurs in " << antCount[j]
                    << " baselines)" << '\n';
        }
      }
    }
    ...
    if (itsStationsToUse[i].size() >= itsMix->minNStation()) {
      ...
      itsSrcSet.push_back(i);
      if (itsMix->verbose() > 10) {
        std::cout << "ignore source " << patchList[i]->name() << " ("
                  << itsStationsToUse[i].size() << " stations)" << '\n';
      }
    }
  }
...
float DemixWorker::findMedian(const Cube<float>& ampl, const bool* selbl) {
...
void DemixWorker::setupDemix(unsigned int chunkNr) {
  ...
  for (unsigned int i = 0; i < nsrc; ++i) {
    ...
    float medAmpl =
        findMedian(itsAteamAmpl[srcOrig], &(itsAteamAmplSel(0, srcOrig)));
  ...
  if (itsMix->targetHandling() == 1) {
    ...
    if (targetMedAmpl / maxMedAmpl > itsMix->ratio1() ||
      ...
      if (itsMix->verbose() > 10) {
        std::cout << "include strong target" << '\n';
      }
    } else if (!itsMix->isAteamNearby()) {
      if (itsMix->verbose() > 10) {
        std::cout << "deproject target" << '\n';
      }
      itsNrDeprojectTarget++;
      ...
      itsNrIncludeCloseTarget++;
      if (itsMix->verbose() > 10) {
        std::cout << "include close target" << '\n';
      }
    } else {
      itsIgnoreTarget = true;
      itsNrIgnoreTarget++;
      if (itsMix->verbose() > 10) {
        std::cout << "ignore target" << '\n';
      }
    }
    if (itsMix->verbose() > 10) {
      std::cout << " targetMedAmpl=" << targetMedAmpl
                << " targetMaxAmpl=" << targetMaxAmpl
                << " maxAteamMedAmpl=" << maxMedAmpl
                << " minAteamMedAmpl=" << minMedAmpl << '\n';
    }
  }
  ...
  for (unsigned int dr = 0; dr < itsNModel; ++dr) {
    ...
    if (itsMix->verbose() > 11) {
      std::cout << "stationstouse " << drOrig << " = "
                << itsStationsToUse[drOrig] << '\n';
    }
      ...
      if (i0 == itsNDir - 1) {
        ...
        for (int i = 0; i < nbl; ++i) {
          ...
          for (int j = 0; j < nchan; ++j) {
            assert(phasor1 < itsPhaseShifts[i1]->getPhasors().data() +
                                 itsPhaseShifts[i1]->getPhasors().size());
            casacore::DComplex factor = conj(*phasor1++);
            for (int k = 0; k < ncorr; ++k) {
              assert(flagPtr < newBuf.GetFlags().data() +
                                   newBuf.GetCasacoreFlags().size());
              assert(weightPtr < newBuf.GetWeights().data() +
                                     newBuf.GetCasacoreWeights().size());
              if (!*flagPtr) {
        ...
        for (int i = 0; i < nbl; ++i) {
          ...
          const casacore::DComplex* phasor1 =
              itsPhaseShifts[i1]->getPhasors().data() + i * nchan;
          for (int j = 0; j < nchan; ++j) {
            casacore::DComplex factor = *phasor0++ * std::conj(*phasor1++);
            for (int k = 0; k < ncorr; ++k) {
              if (!*flagPtr) {
                *factorPtr += factor * double(*weightPtr);
              }
...
void DemixWorker::makeFactors(const Array<casacore::DComplex>& bufIn,
  ...
  bufOut.resize(
      IPosition(5, itsNDir, itsNDir, itsMix->ncorr(), nChanOut, itsMix->nbl()));
    ...
    for (unsigned int d1 = d0 + 1; d1 < itsNDir; ++d1) {
      ...
      for (int k = 0; k < int(itsMix->nbl()); ++k) {
        const casacore::DComplex* phin =
            bufIn.data() + (dirnr * itsMix->nbl() + k) * nccin;
        ...
        for (unsigned int c0 = 0; c0 < nChanOut; ++c0) {
          ...
          unsigned int nch =
              std::min(nChanAvg, itsMix->nchanIn() - c0 * nChanAvg);
  ...
  // std::cout<<"makefactors "<<weightSums<<bufOut;
}

void DemixWorker::deproject(Array<casacore::DComplex>& factors,
...
void DemixWorker::handleDemix(DPBuffer* bufout, std::vector<double>* solutions,
...
void DemixWorker::demix(std::vector<double>* solutions, double time,
                        double timeStep) {
  ...
  for (size_t ts = 0; ts < nTime; ++ts) {
    ...
    if (itsMix->verbose() > 13) {
      std::cout << "modelvis=" << itsModelVisDemix << '\n';
    }
    ...
    // matrix.
    itsTimerSolve.start();
    const_cursor<bool> cr_flag = casa_const_cursor(
        itsAvgResults[itsSrcSet[0]]->get()[ts].GetCasacoreFlags());
    const_cursor<float> cr_weight = casa_const_cursor(
        itsAvgResults[itsSrcSet[0]]->get()[ts].GetCasacoreWeights());
    ...
    for (size_t dr = 0; dr < itsNModel; ++dr) {
      unsigned int drOrig = itsSrcSet[dr];
      cr_data[dr] =
          casa_const_cursor(itsAvgResults[drOrig]->get()[ts].GetCasacoreData());
      cr_model[dr] =
          const_cursor<dcomplex>(itsModelVisDemix[dr].data(), 3, stride_model);
    }
    // If solving the system succeeds, increment nconverged.
    bool converged = itsEstimate.estimate(
        itsUnknownsIndex, itsSrcSet, cr_baseline, cr_data, cr_model, cr_flag,
        cr_weight, cr_mix, itsMix->defaultGain(), itsMix->solveBoth(),
        itsMix->verbose());
    ...
    if (itsMix->doSubtract()) {
      ...
      for (size_t ts_subtr = multiplier * ts,
        ...
        for (size_t bl = 0; bl < nBl; ++bl) {
          ...
          itsObservedAmpl[bl] = 0.5 * (std::abs(data[0]) + std::abs(data[3]));
          data += 4 * nChSubtr;
        }
        for (size_t dr = 0; dr < itsNSubtr; ++dr) {
          unsigned int drOrig = itsSrcSet[dr];
          // Re-use simulation used for estimating Jones matrices if possible.
          cursor<dcomplex> cr_model_subtr(itsModelVisSubtr[dr].data(), 3,
                                          stride_model);
          ...
          cursor<fcomplex> cr_residual =
              casa_cursor(itsAvgResultSubtr->get()[ts_subtr].GetCasacoreData());
          ...
          assert(stride_mix_subtr_slice[0] == itsNDir * itsNDir &&
                 ...
                 stride_mix_subtr_slice[2] ==
                     itsNDir * itsNDir * nCr * nChSubtr);
...
void DemixWorker::addMeanM2(const std::vector<float>& sourceAmpl,
                            unsigned int dr) {
