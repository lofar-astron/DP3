...
class DPInfo {
 public:
  ...
  void SetTelescope(std::shared_ptr<everybeam::Telescope> telescope) {
    telescope_ = std::move(telescope);
  }

  /// @return Telescope reference. Only valid if HasTelescope() returns true.
  const everybeam::Telescope& GetTelescope() const { return *telescope_; }
 ...
 private:
  ...
  std::shared_ptr<everybeam::Telescope> telescope_;
};
