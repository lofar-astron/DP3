...
class DemixInfo {
 public:
  ...
  DemixInfo(const common::ParameterSet&, const string& prefix);
  ...
  const DPInfo& getInfo() const { return itsInfoSel; }
  ...
  static double getCosAngDist(double ra1, double dec1, double ra2,
                              double dec2) {
    return sin(dec1) * sin(dec2) + cos(dec1) * cos(dec2) * cos(ra1 - ra2);
  }

  /// Test if two positions in the sky are within delta radians.
  static bool testAngDist(double ra1, double dec1, double ra2, double dec2,
                          double cosDelta) {
