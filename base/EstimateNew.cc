...
bool EstimateNew::estimate(const vector<vector<int>>& unknownsIndex,
  ...
  while (!solver.isReady() && itsNrIter < itsMaxIter) {
    ...
    for (size_t bl = 0; bl < itsNrBaselines; ++bl) {
      ...
      if (p != q &&
          ((itsSolveStation[p] || itsSolveStation[q]) &&
           (!solveBoth || (itsSolveStation[p] && itsSolveStation[q])))) {
        // Create partial derivative index for current baseline.
        size_t nPartial =
            fillDerivIndex(srcSet.size(), unknownsIndex, *baselines);
        if (verbose > 13) {
          ...
          for (size_t cr = 0; cr < 4; ++cr) {
            ...
            if (!flag[cr]) {
              ...
              for (size_t tg = 0; tg < nDirection; ++tg) {
                ...
                solver.makeNorm(nPartial, &(itsDerivIndex[cr * nPartial]),
                                &(itsdR[0]), static_cast<double>(weight[cr]),
                                real(residual));
                solver.makeNorm(nPartial, &(itsDerivIndex[cr * nPartial]),
                                &(itsdI[0]), static_cast<double>(weight[cr]),
                                imag(residual));
                if (verbose > 14) {
                  cout << "makeres " << real(residual) << ' ' << weight[cr]
                       << ' ' << nPartial;
                  for (unsigned int i = 0; i < nPartial; ++i) {
                    cout << ' ' << itsDerivIndex[cr * nPartial + i] << ' '
                         << itsdR[i];
                  }
                  cout << endl;
                }
    ...
    if (!status) throw std::runtime_error("Solve failed");
