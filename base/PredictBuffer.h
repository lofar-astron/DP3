...
class PredictBuffer {
 public:
  void Resize(size_t n_directions, size_t n_channels, size_t n_stations,
    ...
    if (full_beam) {
      ...
      for (std::vector<aocommon::MC2x2F>& values : full_beam_values_)
  ...
  aocommon::MC2x2F* GetFullBeamValues(size_t direction_index) {
    assert(full_beam_);
 ...
 private:
  std::vector<std::vector<aocommon::MC2x2F>> full_beam_values_;
