...
StationAdder::StationAdder(DPInput* input, const ParameterSet& parset,
                           const string& prefix)
    : itsInput(input),
      itsName(prefix),
...
void StationAdder::updateInfo(const DPInfo& infoIn) {
  ...
  itsUVWCalc =
      UVWCalculator(infoIn.phaseCenter(), infoIn.arrayPos(), antennaPos);
...
bool StationAdder::process(const DPBuffer& buf) {
  ...
  for (unsigned int i = 0; i < itsBufRows.size(); ++i) {
    ...
    for (unsigned int j = 0; j < itsBufRows[i].size(); ++j) {
      ...
      const Complex* inDataPtr = (itsBuf.getData().data() + blnr * nrcc);
    ...
    if (itsDoAverage && uvwWghtSum != 0) {
      ...
      Vector<Double> uvws = itsUVWCalc.getUVW(
