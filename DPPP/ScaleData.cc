...
ScaleData::ScaleData(DPInput*, const ParameterSet& parset, const string& prefix)
    : itsName(prefix),
      itsScaleSizeGiven(False),
      itsScaleSize(False),
      itsStationExp(
          parset.getStringVector(prefix + "stations", vector<string>())),
      itsCoeffStr(parset.getStringVector(prefix + "coeffs", vector<string>())) {
...
bool ScaleData::process(const DPBuffer& buf) {
  ...
  arrayContTransform(static_cast<const Array<Complex>&>(buf.getData()),
                     static_cast<const Array<double>&>(itsFactors), data,
