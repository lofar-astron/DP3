...
BOOST_AUTO_TEST_CASE( test_gridinterpolate ) {
  {
...

    {
      ...
      vector<double> newgridvals = soltab.getValuesOrWeights("val", "Antenna1", times, freqs, 0, 0, true);
      ...
      newgridvals = soltab.getValuesOrWeights("val", "Antenna1", times, freqs, 0, 0, true);
      BOOST_CHECK_EQUAL(newgridvals.size(), times.size() * freqs.size());
      idx=0;
      for (int time=0; time<int(times.size()); ++time) {
        for (size_t freq=0; freq<freqs.size(); ++freq) {
          BOOST_CHECK(casacore::near(newgridvals[idx++], min(double((time+1)/3),double(ntimes-1))));
        }
      }
