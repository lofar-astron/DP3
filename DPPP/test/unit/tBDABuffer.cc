...
BOOST_AUTO_TEST_CASE(copy) {
  ...
  BOOST_CHECK(buffer.AddRow(kTime, kInterval, kRowNr, kBaselineNr, kDataSize,
                            kData1, nullptr, kWeights1, kFlags, kUvw));
  BOOST_CHECK(buffer.AddRow(kTime + 1., kInterval + 1., kRowNr + 1,
                            kBaselineNr + 1, kDataSize, kData2, kFlags,
  ...
  BOOST_CHECK_EQUAL(rows_copy[0].n_elements, kDataSize);
  ...
  BOOST_CHECK_EQUAL(rows_copy[1].n_elements, kDataSize);
...
BOOST_AUTO_TEST_CASE(add_all_fields) {
  ...
  BOOST_CHECK(buffer.AddRow(kTime + 1., kInterval + 1., kRowNr + 1,
                            kBaselineNr + 1, k1DataSize, kData2, kFlags2,
  ...
  BOOST_CHECK_EQUAL(rows[0].n_elements, kDataSize);
  ...
  BOOST_CHECK_EQUAL(rows[1].n_elements, k1DataSize);
...
BOOST_AUTO_TEST_CASE(add_no_fields) {
  ...
  BOOST_CHECK_EQUAL(row.n_elements, kDataSize);
...
BOOST_AUTO_TEST_CASE(add_wrong_ordering) {
  ...
  BOOST_CHECK(
      buffer.AddRow(kTime1, kInterval1, kRowNr, kBaselineNr, kDataSize));
  // end time equals start time of last row -> invalid ordering.
  BOOST_CHECK_THROW(
      buffer.AddRow(kTime2, kInterval2, kRowNr, kBaselineNr, kDataSize),
...
BOOST_AUTO_TEST_CASE(clear) {
  ...
  BOOST_CHECK(buffer.AddRow(kTime, kInterval, kRowNr, kBaselineNr, kDataSize));
  BOOST_CHECK(
      buffer.AddRow(kTime, kInterval, kRowNr, kBaselineNr + 1, kDataSize));
  BOOST_CHECK(
      buffer.AddRow(kTime, kInterval, kRowNr, kBaselineNr + 2, kDataSize));
  BOOST_CHECK(
      !buffer.AddRow(kTime, kInterval, kRowNr, kBaselineNr + 3, kDataSize));
