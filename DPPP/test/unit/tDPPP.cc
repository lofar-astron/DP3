...
void checkCopy(const String& in, const String& out, int nms) {
  ...
  for (int j = 0; j < nms; ++j) {
    ...
    BOOST_CHECK(
        allEQ(ROScalarColumn<double>(t1, "TIME_CENTROID").getColumn(),
  ...
  for (int j = 0; j < nms; ++j) {
    ...
    Slicer dslicer(dst, dshape);
    BOOST_CHECK(
        allEQ(ROArrayColumn<double>(spwin, "CHAN_FREQ").getColumn(),
              ROArrayColumn<double>(spwout, "CHAN_FREQ").getColumn(dslicer)));
    BOOST_CHECK(
        allEQ(ROArrayColumn<double>(spwin, "CHAN_WIDTH").getColumn(),
              ROArrayColumn<double>(spwout, "CHAN_WIDTH").getColumn(dslicer)));
    BOOST_CHECK(allEQ(
        ROArrayColumn<double>(spwin, "EFFECTIVE_BW").getColumn(),
        ROArrayColumn<double>(spwout, "EFFECTIVE_BW").getColumn(dslicer)));
    BOOST_CHECK(
        allEQ(ROArrayColumn<double>(spwin, "RESOLUTION").getColumn(),
              ROArrayColumn<double>(spwout, "RESOLUTION").getColumn(dslicer)));
  }
  BOOST_CHECK(
      allEQ(double(nms) *
                ROScalarColumn<double>(spwin, "TOTAL_BANDWIDTH").getColumn(),
            ROScalarColumn<double>(spwout, "TOTAL_BANDWIDTH").getColumn()));
  if (nms == 1) {
    BOOST_CHECK(
        allEQ(ROScalarColumn<double>(spwin, "REF_FREQUENCY").getColumn(),
  ...
  BOOST_CHECK(
      near(timeRange(1), ROScalarColumn<double>(tout, "TIME")(143) + 15));
}
...
void testMultiIn() {
  ...
  BOOST_CHECK(
      allEQ(ROArrayColumn<Complex>(tab, "DATA").getColumn(), Complex()));
...
void checkAvg(const String& outName) {
  ...
  while (!iterin.pastEnd()) {
    ...
    BOOST_CHECK(allEQ(flag.getColumn(), false));
    BOOST_CHECK(
        allNear(ROScalarColumn<double>(t1, "TIME").getColumn(), time, 1e-5));
    BOOST_CHECK(allNear(ROScalarColumn<double>(t1, "TIME_CENTROID").getColumn(),
                        time, 1e-5));
    BOOST_CHECK(
        allEQ(ROScalarColumn<double>(t1, "INTERVAL").getColumn(), 20 * 30.));
    BOOST_CHECK(
        allEQ(ROScalarColumn<double>(t1, "EXPOSURE").getColumn(), 20 * 30.));
  ...
  BOOST_CHECK(near(
      cw(0, 0), sum(ROArrayColumn<double>(spwin, "CHAN_WIDTH").getColumn())));
  ...
  BOOST_CHECK(
      near(ROScalarColumn<double>(spwin, "TOTAL_BANDWIDTH")(0), cw(0, 0)));
  BOOST_CHECK(
      allEQ(ROScalarColumn<double>(spwin, "REF_FREQUENCY").getColumn(),
            ROScalarColumn<double>(spwout, "REF_FREQUENCY").getColumn()));
  // Check the TIME_RANGE in the OBSERVATION table.
  ...
  Vector<double> timeRange(
      ROArrayColumn<double>(obsout, "TIME_RANGE").getColumn());
  BOOST_CHECK(
      near(timeRange(0), ROScalarColumn<double>(tout, "TIME")(0) - 300));
  BOOST_CHECK(
      near(timeRange(1), ROScalarColumn<double>(tout, "TIME")(0) + 295));
}
...
void testUpdateScale() {
  ...
  // Check that all data is doubled.
  {
    ...
    data *= Complex(2, 0);
    BOOST_CHECK(
        allNear(ROArrayColumn<Complex>(tab, "DATA").getColumn(), data, 1e-5));
    BOOST_CHECK(allEQ(ROArrayColumn<bool>(tab, "FLAG").getColumn(), flags));
  }
}
...
void testStationAdd() {
  ...
  BOOST_CHECK_EQUAL(t2.nrow(), t1.nrow() + 1);  // 1 antenna has been added
  BOOST_CHECK_EQUAL(ROScalarColumn<Int>(t2, "ANTENNA_ID")(t2.nrow() - 1),
                    oldNant);
...
void testFilter2() {
  ...
  for (int iter = 0; iter < 4; ++iter) {
    ...
    for (int i = 0; i < 6; ++i) {
      ...
      BOOST_CHECK(
          allEQ(ROScalarColumn<Int>(t2s, "ANTENNA1").getColumn(), ant1[i]));
...
void testMultiOut() {
  ...
  // Check that tables exist, contain the specified columns
  {
    ...
    BOOST_CHECK(allNear(
        ROArrayColumn<Complex>(tab2, "DATA").getColumn(),
        Complex(2, 0) * ROArrayColumn<Complex>(tab1, "DATA").getColumn(),
        1e-5));
    BOOST_CHECK(allNear(
        ROArrayColumn<Complex>(tab2, "DATA_2").getColumn(),
        Complex(4, 0) * ROArrayColumn<Complex>(tab1, "DATA").getColumn(),
        1e-5));
    BOOST_CHECK(allNear(
        ROArrayColumn<Float>(tab2, "WEIGHT_SPECTRUM").getColumn(),
        ROArrayColumn<Float>(tab1, "WEIGHT_SPECTRUM").getColumn(), 1e-5));
    ...
    BOOST_CHECK(allNear(
        ROArrayColumn<Complex>(tab3, "DATA").getColumn(),
        Complex(4, 0) * ROArrayColumn<Complex>(tab1, "DATA").getColumn(),
        1e-5));
    BOOST_CHECK(allNear(
        ROArrayColumn<Float>(tab3, "WEIGHT_SPECTRUM").getColumn(),
        ROArrayColumn<Float>(tab1, "WEIGHT_SPECTRUM").getColumn(), 1e-5));
...
void testErrorOut() {
  ...
  }
  {
    ...
    ostr << "out2.name=./tNDPPP_tmp.MSx"
         << endl;  // update not possible (filter)
    ostr << "msout=''" << endl;
    tryErr("tNDPPP_tmp.parset");
  }
}
...
BOOST_AUTO_TEST_CASE(test_copy_column, * utf::depends_on("dppp/test_copy")) { testCopyColumn(); }

BOOST_AUTO_TEST_CASE(test_multi_in, * utf::depends_on("dppp/test_copy_column")) { testMultiIn(); }

BOOST_AUTO_TEST_CASE(test_avg1, * utf::depends_on("dppp/test_multi_in")) { testAvg1(); }

BOOST_AUTO_TEST_CASE(test_avg2, * utf::depends_on("dppp/test_avg1")) { testAvg2(); }

BOOST_AUTO_TEST_CASE(test_avg3, * utf::depends_on("dppp/test_avg2")) { testAvg3(); }

BOOST_AUTO_TEST_CASE(test_avg4, * utf::depends_on("dppp/test_avg3")) { testAvg4(); }

BOOST_AUTO_TEST_CASE(test_update1, * utf::depends_on("dppp/test_avg4")) { testUpdate1(); }

BOOST_AUTO_TEST_CASE(test_update2, * utf::depends_on("dppp/test_update1")) { testUpdate2(); }

BOOST_AUTO_TEST_CASE(test_update_scale, * utf::depends_on("dppp/test_update2")) { testUpdateScale(); }

BOOST_AUTO_TEST_CASE(test_flags1, * utf::depends_on("dppp/test_update_scale")) { testFlags1(); }

BOOST_AUTO_TEST_CASE(test_flags2, * utf::depends_on("dppp/test_flags1")) { testFlags2(); }

BOOST_AUTO_TEST_CASE(test_flags3, * utf::depends_on("dppp/test_flags2")) { testFlags3(); }

BOOST_AUTO_TEST_CASE(test_station_add, * utf::depends_on("dppp/test_flags3")) { testStationAdd(); }

BOOST_AUTO_TEST_CASE(test_filter1, * utf::depends_on("dppp/test_station_add")) { testFilter1(); }

BOOST_AUTO_TEST_CASE(test_filter2, * utf::depends_on("dppp/test_filter1")) { testFilter2(); }

BOOST_AUTO_TEST_CASE(test_filter3, * utf::depends_on("dppp/test_filter2")) { testFilter3(); }

BOOST_AUTO_TEST_CASE(test_clear, * utf::depends_on("dppp/test_filter3")) { testClear(); }

BOOST_AUTO_TEST_CASE(test_multi_out, * utf::depends_on("dppp/test_clear")) { testMultiOut(); }

BOOST_AUTO_TEST_CASE(test_error_out, * utf::depends_on("dppp/test_multi_out")) { testErrorOut(); }
