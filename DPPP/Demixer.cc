...
Demixer::Demixer(DPInput* input, const ParameterSet& parset,
                 const string& prefix)
    : itsInput(input),
      itsName(prefix),
      ...
      itsModelSources(
          parset.getStringVector(prefix + "modelsources", vector<string>())),
      itsExtraSources(
          parset.getStringVector(prefix + "othersources", vector<string>())),
...
void Demixer::show(std::ostream& os) const {
  ...
  if (itsSelBL.hasSelection()) {
    os << "    demixing " << itsFilter.getInfo().nbaselines() << " out of "
       ...
       << getInfo().antennaUsed().size() << " stations)" << std::endl;
  }
  ...
  //      endl; os << "  Solve.Options.EpsValue:      " << itsSolveOpt.epsValue
  //      << endl; os << "  Solve.Options.EpsDerivative: " <<
  //      itsSolveOpt.epsDerivative << endl; os << "  Solve.Options.ColFactor:
  //      " << itsSolveOpt.colFactor << endl; os << "  Solve.Options.LMFactor:
  //      " << itsSolveOpt.lmFactor << endl; os << "  Solve.Options.BalancedEqs:
  //      " << itsSolveOpt.balancedEq << endl; os << "  Solve.Options.UseSVD: "
  //      << itsSolveOpt.useSVD <<endl;
}
...
void Demixer::finish() {
  cerr << "  " << itsNTimeIn << " time slots to finish in Demixer ..." << endl;
  itsTimer.start();
...
void Demixer::addFactors(const DPBuffer& newBuf, Array<DComplex>& factorBuf) {
...
void Demixer::makeFactors(const Array<DComplex>& bufIn, Array<DComplex>& bufOut,
...
void Demixer::demix() {
  ...
  loop.Run(0, nTime, [&](size_t ts, size_t thread) {
    ...
    for (size_t dr = 0; dr < nDr; ++dr) {
      cr_data[dr] = casa_const_cursor(itsAvgResults[dr]->get()[ts].getData());
      cr_model[dr] =
          const_cursor<dcomplex>(storage.model[dr].data(), 3, stride_model);
    }
    ...
    for (size_t ts_subtr = multiplier * ts,
      ...
      for (size_t dr = 0; dr < nDrSubtr; ++dr) {
        ...
        cursor<dcomplex> cr_model_subtr(storage.model[dr].data(), 3,
                                        stride_model);

        // Re-simulate if required.
        if (multiplier != 1 || nCh != nChSubtr) {
          nsplitUVW(itsUVWSplitIndex, itsBaselines,
                    itsAvgResultSubtr->get()[ts_subtr].getUVW(), storage.uvw);
          ...
          Simulator simulator(itsPatchList[dr]->position(), nSt, nBl, nChSubtr,
        ...
        const_cursor<double> cr_unknowns(&(storage.unknowns[dr * nSt * 8]), 2,
                                         stride_unknowns);
        ...
        const_cursor<dcomplex> cr_mix_subtr = const_cursor<dcomplex>(
...
void Demixer::dumpSolutions() {
  ...
  BBS::Axis::ShPtr freqAxis(
      new BBS::RegularAxis(freq[0] - freqWidth[0] * 0.5, freqWidth[0], 1));
