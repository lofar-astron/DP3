...
namespace DP3 {
H5Parm::SolTab::SolTab(H5::Group group, const string& type,
                       const vector<AxisInfo> axes)
    : H5::Group(group), _type(type), _axes(axes) {
  H5::Attribute attr = createAttribute(
      "TITLE", H5::StrType(H5::PredType::C_S1, _type.size()), H5::DataSpace());
  attr.write(H5::StrType(H5::PredType::C_S1, _type.size()), _type);
  addVersionStamp(*this);
}

H5Parm::SolTab::SolTab(H5::Group& group) : H5::Group(group) {
...
bool H5Parm::SolTab::hasAxis(const string& axisName) {
  for (size_t i = 0; i < _axes.size(); ++i) {
    if (_axes[i].name == axisName) return true;
  }
  return false;
}
...
void H5Parm::SolTab::readAxes() {
  ...
  if (ds.getSimpleExtentNdims() != int(ndims))
    throw std::runtime_error(
        "H5Parm is inconsistent: number of axes in data does not match number "
        "of axes in metadata");
...
vector<double> H5Parm::SolTab::getValuesOrWeights(
    const string& valOrWeight, const string& antName,
    const vector<double>& times, const vector<double>& freqs, unsigned int pol,
  ...
  if (hasAxis("freq")) {
    ...
    freqAxisH5 = vector<double>(fullFreqAxisH5.begin() + startFreq,
                                fullFreqAxisH5.begin() + startFreq + nfreqH5);
  }
  if (hasAxis("pol")) {
    unsigned int npolH5 = getAxis("pol").size;
    if (pol > npolH5 - 1) {
      throw Exception("Polarization " + std::to_string(pol) +
                      " requested from H5Parm, but only " +
                      std::to_string(npolH5) + " polarizations are in there.");
    }
  }

  vector<double> h5values =
      getValuesOrWeights(valOrWeight, antName, startTimeSlot, ntimeH5, 1,
                         startFreq, nfreqH5, 1, pol, dir);

  gridNearestNeighbor(timeAxisH5, freqAxisH5, times, freqs, h5values.data(),
                      interpolated.data(), nearest);
...
vector<double> H5Parm::SolTab::getValuesOrWeights(
  ...
  for (unsigned int i = 0; i < _axes.size(); ++i) {
    ...
    if (_axes[i].name == "time") {
      ...
      if (_axes[i].size != 1)
        throw std::runtime_error("Axis \"" + _axes[i].name +
...
void H5Parm::SolTab::setAntennas(const vector<string>& solAntennas) {
  ...
  H5::DataSet dataset =
      createDataSet("ant", H5::StrType(H5::PredType::C_S1, 16), dataspace);
...
void H5Parm::SolTab::setAxisMeta(const string& metaName, size_t nChar,
  ...
  H5::DataSet dataset = createDataSet(
      metaName, H5::StrType(H5::PredType::C_S1, nChar), dataspace);
...
void H5Parm::SolTab::setAxisMeta(const string& metaName,
  ...
  H5::DataSpace dataspace(1, dims, NULL);
  H5::DataSet dataset =
      createDataSet(metaName, H5::PredType::IEEE_F64LE, dataspace);
...
void H5Parm::SolTab::fillCache(map<string, hsize_t>& cache,
  ...
  if (dataspace.getSimpleExtentNdims() != 1)
    throw std::runtime_error("Invalid H5Parm: table \"" + tableName +
...
hsize_t H5Parm::SolTab::getNamedIndex(map<string, hsize_t>& cache,
  ...
  if (it == cache.end()) {
    throw Exception("SolTab has no element " + elementName + " in " +
                    tableName);
  }
...
hsize_t H5Parm::SolTab::getFreqIndex(double freq) const {
  ...
  throw Exception("Frequency " + std::to_string(freq) + " not found in " +
                  getName());
  return 0;
}
...
vector<double> H5Parm::SolTab::getRealAxis(const string& axisname) const {
  ...
  if (dataspace.getSimpleExtentNdims() != 1)
    throw std::runtime_error(
        "Something wrong with H5Parm: dataspace.getSimpleExtentNdims()!=1");
...
hsize_t H5Parm::SolTab::getTimeIndex(double time) const {
  ...
  throw Exception("Time " + std::to_string(time) + " not found in " +
                  getName());
  return 0;
}
...
double H5Parm::SolTab::getInterval(const string& axisName, size_t start) const {
  ...
  try {
    ...
    throw Exception("SolTab " + getName() + " has no axis table for " +
                    axisName);
  }
  if (dataspace.getSimpleExtentNdims() != 1)
    throw std::runtime_error("Invalid H5Parm: table \"" + axisName +
                             "\" should be onedimensional");

  hsize_t dims[1];
  dataspace.getSimpleExtentDims(dims);
  if (dims[0] <= start + 1)
    throw Exception("For reading the " + axisName +
