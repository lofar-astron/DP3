...
DemixWorker::DemixWorker(DPInput* input, const string& prefix,
  ...
  for (unsigned int i = 0; i < nsrc; ++i) {
    ...
    PhaseShift* step1 =
        new PhaseShift(input, ParameterSet(),
...
unsigned int DemixWorker::avgSplitUVW(const DPBuffer* bufin,
                                      ...
                                      const vector<unsigned int>& selbl) {
    ...
    for (unsigned int j = 0; j < itsMix->nbl(); ++j, miter.next()) {
      if (itsMix->verbose() > 11) {
        cout << "source " << i << ", baseline " << itsMix->getAnt1()[j] << ' '
             << itsMix->getAnt2()[j] << " has max ampl " << max(miter.matrix())
             << endl;
      }
...
void DemixWorker::demix(vector<double>* solutions, double time,
                        double timeStep) {
