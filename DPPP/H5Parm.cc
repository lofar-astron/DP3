...
namespace DP3 {
H5Parm::H5Parm(const std::string& filename, bool forceNew, bool forceNewSolSet,
  ...
  if (forceNewSolSet || getNumObjs() == 0) {  // Create a new solSet
    ...
    if (solSetNameToOpen == "") {
      if (this->getNumObjs() == 1) {
        ...
        throw Exception("H5Parm " + filename +
    ...
    for (vector<string>::iterator solTabName = solTabNames.begin();
      ...
      _solTabs.insert(std::map<std::string, SolTab>::value_type(*solTabName,
                                                                SolTab(group)));
    }
  }
}
...
void H5Parm::addSources(const std::vector<std::string>& names,
  ...
  sourceType.insertMember("name", HOFFSET(antenna_t, name),
                          H5::StrType(H5::PredType::C_S1, 128));
  sourceType.insertMember("dir", HOFFSET(source_t, dir),
...
void H5Parm::addAntennas(const std::vector<std::string>& names,
  ...
  antennaType.insertMember("name", HOFFSET(antenna_t, name),
                           H5::StrType(H5::PredType::C_S1, 16));
  antennaType.insertMember("position", HOFFSET(antenna_t, position),
                           H5::ArrayType(H5::PredType::NATIVE_FLOAT, 1, dims));
  ...
  H5::DataSpace dataspace(1, dims, NULL);
  H5::DataSet dataset =
      _solSet.createDataSet("antenna", antennaType, dataspace);
...
H5Parm::SolTab& H5Parm::getSolTab(const std::string& name) {
  std::map<std::string, SolTab>::iterator item = _solTabs.find(name);
