...
UVWFlagger::UVWFlagger(DPInput* input, const ParameterSet& parset,
                       const string& prefix)
    : itsInput(input),
      itsName(prefix),
      itsNTimes(0),
  ...
  itsIsDegenerate = itsRangeUVm.size() + itsRangeUVl.size() +
                    ...
                    0;
  itsCenter = parset.getStringVector(prefix + "phasecenter", vector<string>());
}
...
void UVWFlagger::handleCenter() {
  ...
  itsUVWCalc =
      UVWCalculator(phaseCenter, getInfo().arrayPos(), getInfo().antennaPos());
}
