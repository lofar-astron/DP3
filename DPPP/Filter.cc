...
Filter::Filter(DPInput* input, const ParameterSet& parset, const string& prefix)
    : itsInput(input),
      itsName(prefix),
...
bool Filter::process(const DPBuffer& buf) {
  ...
  const Array<Double>& uvws = itsInput->fetchUVW(buf, itsBufTmp, itsTimer);
...
void Filter::addToMS(const string& msName) {
  ...
  rownr_t nrAntFldId;
  Vector<rownr_t> remAntFldId = renumberSubTable(
...
Vector<int> Filter::createIdMap(rownr_t nrId,
  ...
  for (rownr_t j = removedIds[removedIds.size() - 1] + 1; j < idMap.size();
       ++j) {
    idMap[j] -= nrrem;
  }
  return idMap;
...
Vector<rownr_t> Filter::renumberSubTable(const Table& ms, const String& name,
