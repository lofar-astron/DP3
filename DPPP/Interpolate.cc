...
void register_interpolate() {
  DP3::DPPP::DPRun::registerStepCtor("interpolate",
                                     DP3::DPPP::Interpolate::makeStep);
}

namespace DP3 {
namespace DPPP {

Interpolate::Interpolate(DPInput* /*input*/, const ParameterSet& parset,
                         const string& prefix)
    : _name(prefix),
      _interpolatedPos(0),
...
void Interpolate::interpolateSample(size_t timestep, size_t baseline,
  ...
  for (size_t t = timestepBegin; t != timestepEnd; ++t) {
    Complex* data = _buffers[t].getData().data() +
  ...
  Complex& value = _buffers[timestep]
                       .getData()
                       .data()[(baseline * nChan + channel) * nPol + pol];
  if (windowSum != 0.0)
    value = valueSum / windowSum;
  else
    value = Complex(std::numeric_limits<float>::quiet_NaN(),
