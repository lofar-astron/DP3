...
GainCal::GainCal(DPInput* input, const ParameterSet& parset,
                 const string& prefix)
    : itsInput(input),
      itsName(prefix),
    ...
    for (unsigned int bl = 0; bl < nBl; ++bl) {
      if (itsSelectedBL[bl]) {
        ...
        for (unsigned int cr = 0; cr < nCr; ++cr) {
          ...
          iS[ch / itsNChan].getVis()(IPosition(
              6, ant1, cr / nCrDiv, itsStepInSolInt, ch % itsNChan, cr % 2,
              ant2)) = DComplex(data[bl * nCr * nCh + ch * nCr + cr]) *
                       DComplex(sqrt(weight[bl * nCr * nCh + ch * nCr + cr]));
          iS[ch / itsNChan].getMVis()(IPosition(
              6, ant1, cr / nCrDiv, itsStepInSolInt, ch % itsNChan, cr % 2,
              ant2)) = DComplex(model[bl * nCr * nCh + ch * nCr + cr]) *
                       DComplex(sqrt(weight[bl * nCr * nCh + ch * nCr + cr]));

          // conjugate transpose
          iS[ch / itsNChan].getVis()(IPosition(
              6, ant2, cr % 2, itsStepInSolInt, ch % itsNChan, cr / nCrDiv,
              ant1)) = DComplex(conj(data[bl * nCr * nCh + ch * nCr + cr])) *
                       DComplex(sqrt(weight[bl * nCr * nCh + ch * nCr + cr]));
          iS[ch / itsNChan].getMVis()(IPosition(
              6, ant2, cr % 2, itsStepInSolInt, ch % itsNChan, cr / nCrDiv,
              ant1)) = DComplex(conj(model[bl * nCr * nCh + ch * nCr + cr])) *
                       DComplex(sqrt(weight[bl * nCr * nCh + ch * nCr + cr]));
        }
      }
    }
...
void GainCal::calibrate() {
  ...
  vector<GainCalAlgorithm::Status> converged(itsNFreqCells,
                                             GainCalAlgorithm::NOTCONVERGED);
        ...
        for (unsigned int freqCell = 0; freqCell < itsNFreqCells; ++freqCell) {
          if (iS[freqCell].getStationFlagged()[st % nSt] ||
            ...
            if (!isFinite(phases[freqCell])) {
              cout << "Yuk, phases[freqCell]=" << phases[freqCell]
                   << ", sols_f(freqCell, st)=" << sols_f(freqCell, st) << endl;
...
void GainCal::initParmDB() {
  itsParmDB = std::shared_ptr<BBS::ParmDB>(
      new BBS::ParmDB(BBS::ParmDBMeta("casa", itsParmDBName), false));
  itsParmDB->lock();
  ...
  if (!itsParmDB->getNames(parmname).empty()) {
    ...
    BBS::Axis::ShPtr fdomAxis(
        new BBS::RegularAxis(info().chanFreqs()[0] - freqWidth * 0.5,
                             freqWidth * getInfo().chanFreqs().size(), 1));

    itsParmDB->deleteValues(parmname,
                            BBS::Box(fdomAxis->start(), tdomAxis->start(),
...
void GainCal::writeSolutionsH5Parm(double) {
  ...
  if (itsMode == TEC || itsMode == TECANDPHASE) {
    vector<double> tecsols(nSolFreqs * antennaUsedNames.size() * nSolTimes *
                           nPol);
...
void GainCal::writeSolutionsParmDB(double startTime) {
  ...
  double endTime =
      min(startTime + ntime * info().timeInterval() * itsSolInt,
  ...
  for (unsigned int t = 0; t < ntime; ++t) {
    ...
    hightimes[t] =
        min(startTime + info().timeInterval() * itsSolInt * (t + 1), endTime);
  }
  BBS::Axis::ShPtr timeAxis = Axis::makeAxis(lowtimes, hightimes);

  BBS::Axis::ShPtr freqAxis(new BBS::RegularAxis(
  ...
  BBS::Axis::ShPtr tdomAxis(
      new BBS::RegularAxis(startTime, endTime - startTime, 1));
  BBS::Axis::ShPtr fdomAxis(
      new BBS::RegularAxis(info().chanFreqs()[0] - freqWidth * 0.5,
                           freqWidth * getInfo().chanFreqs().size(), 1));
  ...
  for (size_t st = 0; st < nSt; ++st) {
    ...
    for (int pol = 0; pol < 4; ++pol) {  // For 0101
      ...
      for (int realim = 0; realim < realimmax;
        ...
        BBS::ParmValueSet pvs(domainGrid, vector<BBS::ParmValue::ShPtr>(1, pv));
