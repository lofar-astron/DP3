...
MedFlagger::MedFlagger(DPInput* input, const ParameterSet& parset,
                       const string& prefix)
    : itsInput(input),
      itsName(prefix),
      ...
      itsMedianTime(0) {
  itsFlagCorr =
      parset.getUintVector(prefix + "correlations", vector<unsigned int>());
...
void MedFlagger::flag(unsigned int index,
                      const vector<unsigned int>& timeEntries) {
