...
PreFlagger::PreFlagger(DPInput* input, const ParameterSet& parset,
                       const string& prefix)
    : itsName(prefix),
      itsInput(input),
      itsMode(SetFlag),
  ...
  for (unsigned int i = 0; i < nrbl; ++i) {
    for (unsigned int j = 0; j < nrchan; ++j) {
      if (*inPtr == mode) {
        ...
        for (unsigned int k = 0; k < nrcorr; ++k) {
          if (!isFinite(dataPtr[k].real()) || !isFinite(dataPtr[k].imag()) ||
              weightPtr[k] == 0) {
            flag = true;
            break;
          }
...
PreFlagger::PSet::PSet(DPInput* input, const ParameterSet& parset,
                       const string& prefix)
    : itsInput(input),
      itsName(prefix),
      itsFlagOnUV(false),
  ...
  itsStrTime = parset.getStringVector(prefix + "timeofday", vector<string>());
  itsStrLST = parset.getStringVector(prefix + "lst", vector<string>());
  itsStrATime = parset.getStringVector(prefix + "abstime", vector<string>());
  itsStrRTime = parset.getStringVector(prefix + "reltime", vector<string>());
  itsTimeSlot = parset.getUintVector(
      prefix + "timeslot", vector<unsigned int>(), true);  // expand ..
  itsStrAzim = parset.getStringVector(prefix + "azimuth", vector<string>());
  itsStrElev = parset.getStringVector(prefix + "elevation", vector<string>());
  itsMinUV = parset.getDouble(prefix + "uvmmin", -1);
  itsMaxUV = parset.getDouble(prefix + "uvmmax", -1);
  itsStrFreq = parset.getStringVector(prefix + "freqrange", vector<string>());
  ...
  if (!itsPSets.empty()) {
    ...
    for (int oper : itsRpn) {
      if (oper >= 0) {
        ...
        // No ||= operator exists, so use the transform function.
        if (oper == OpOr) {
          transformInPlace(left->cbegin(), left->cend(), right->cbegin(),
                           std::logical_or<bool>());
        } else {
          transformInPlace(left->cbegin(), left->cend(), right->cbegin(),
    ...
    transformInPlace(itsFlags.cbegin(), itsFlags.cend(), mflags->cbegin(),
                     std::logical_and<bool>());
  }
  return &itsFlags;
}
...
void PreFlagger::PSet::flagChannels() {
  ...
  bool* flagPtr = itsFlags.data();
  for (unsigned int i = 0; i < nrbl; ++i) {
    transformInPlace(flagPtr, flagPtr + nr, itsChanFlags.cbegin(),
                     std::logical_and<bool>());
    flagPtr += nr;
  }
}
...
vector<double> PreFlagger::PSet::fillTimes(const vector<string>& vec,
  ...
  for (vector<string>::const_iterator str = vec.begin(); str != vec.end();
       ++str) {
    // Find the .. or +- token.
    bool usepm = false;
...
vector<float> PreFlagger::PSet::fillValuePerCorr(const ParameterValue& value,
