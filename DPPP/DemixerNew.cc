...
DemixerNew::DemixerNew(DPInput* input, const ParameterSet& parset,
                       const string& prefix)
    : itsInput(input),
      itsName(prefix),
...
void DemixerNew::writeSolutions(double startTime, int ntime) {
  ...
  BBS::Axis::ShPtr timeAxis(
      new BBS::RegularAxis(startTime, itsDemixInfo.timeIntervalAvg(), ntime));
  ...
  if (!itsParmDB) {
    itsParmDB = std::shared_ptr<BBS::ParmDB>(
        new BBS::ParmDB(BBS::ParmDBMeta("casa", itsInstrumentName), true));
    itsParmDB->lock();
