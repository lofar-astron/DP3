...
class H5Parm : private H5::H5File {
 ...
 public:
  ...
  struct AxisInfo {
   public:
    AxisInfo(const std::string& name, unsigned int size)
        : name(name), size(size){};
  ...
  class SolTab : private H5::Group {
   public:
    ...
    void setAxisMeta(const std::string& metaName, size_t strLen,
    ...
    void setComplexValues(const std::vector<std::complex<double> >& vals,
                          const std::vector<double>& weights, bool toAmplitudes,
                          const std::string& history = "");
    ...
    std::vector<double> getValues(const std::string& antName,
                                  unsigned int starttimeslot,
                                  unsigned int ntime, unsigned int timestep,
                                  unsigned int startfreq, unsigned int nfreq,
                                  unsigned int freqstep, unsigned int pol,
                                  unsigned int dir) {
      return getValuesOrWeights("val", antName, starttimeslot, ntime, timestep,
                                startfreq, nfreq, freqstep, pol, dir);
    }

    /// Get the weights of this SolTab for a given antenna.
    std::vector<double> getWeights(const std::string& antName,
                                   unsigned int starttimeslot,
                                   unsigned int ntime, unsigned int timestep,
                                   unsigned int startfreq, unsigned int nfreq,
                                   unsigned int freqstep, unsigned int pol,
                                   unsigned int dir) {
      return getValuesOrWeights("weight", antName, starttimeslot, ntime,
                                timestep, startfreq, nfreq, freqstep, pol, dir);
    }

    std::vector<double> getValuesOrWeights(const std::string& valOrWeight,
   ...
   private:
    ...
    std::vector<double> getValuesOrWeights(
        const std::string& valOrWeight, const std::string& antName,
