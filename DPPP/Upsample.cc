...
Upsample::Upsample(DPInput*, const ParameterSet& parset, const string& prefix)
...
bool Upsample::process(const DPBuffer& bufin) {
  ...
  for (unsigned int prevIndex = itsFirstToFlush; prevIndex < itsTimeStep;
    ...
    while (
        itsPrevBuffers[prevIndex].getTime() > itsBuffers[curIndex].getTime() &&
        !nearAbs(itsPrevBuffers[prevIndex].getTime(),
                 itsBuffers[curIndex].getTime(), 0.4 * info().timeInterval())) {
      curIndex++;
    }
