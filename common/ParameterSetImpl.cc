...
//
std::shared_ptr<ParameterSetImpl> ParameterSetImpl::makeSubset(
  ...
  std::string base =
      (itsMode == KeyCompare::NOCASE) ? boost::to_lower_copy(baseKey) : baseKey;
  ...
  for (const_iterator it = lower_bound(base); it != end(); ++it) {
    ...
    pos = subSet->insert(
        pos, make_pair(prefix + it->first.substr(base.size()), it->second));
...
void ParameterSetImpl::subtractSubset(const std::string& fullPrefix) {
  ...
  std::string prefix = (itsMode == KeyCompare::NOCASE)
                           ? boost::to_lower_copy(fullPrefix)
                           : fullPrefix;
...
void ParameterSetImpl::adoptCollection(const ParameterSetImpl& theCollection,
  ...
  if (&theCollection != this) {
    ...
    for (const_iterator iter = tmp.begin(); iter != tmp.end(); ++iter) {
      replaceUnlocked(thePrefix + iter->first, iter->second);
    }
  }
}
...
// (private)
//
// Disentangles the file and adds the Key-Values pair to the current
// ParameterSetImpl.
//
void ParameterSetImpl::readFile(const std::string& theFilename,
                                const std::string& prefix, const bool merge) {
...
void ParameterSetImpl::readStream(std::istream& inputStream,
                                  const std::string& prefix, bool merge) {
  // Thread-safety.
  std::lock_guard<std::mutex> locker(itsMutex);
        ...
        while (st < end) {
          if (line[st] == '"' || line[st] == '\'') {
            ...
            if (line[st] == '=') {
              ...
              ParameterValue pvkey(line.substr(stval, nonbl - stval + 1),
                                   false);
              key = pvkey.getString();
              if (key.empty())
...
// findKV(key) [private]
//
ParameterSetImpl::const_iterator ParameterSetImpl::findKV(
    const std::string& aKey, bool doThrow) const {
  // Thread-safety.
  std::lock_guard<std::mutex> locker(itsMutex);
...
// add (key, value)
//
void ParameterSetImpl::add(const std::string& aKey,
...
// replace (key, value)
//
void ParameterSetImpl::replace(const std::string& aKey,
                               const ParameterValue& aValue) {
  std::lock_guard<std::mutex> locker(itsMutex);
...
#define PARAMETERSETIMPL_GETVECTOR(TPC, TPL)                                   \
  std::vector<TPL> ParameterSetImpl::get##TPC##Vector(const std::string& aKey, \
    ...
    return value.get##TPC##Vector();                                           \
                                                                               ...
                                                                               \
  std::vector<TPL> ParameterSetImpl::get##TPC##Vector(                         \
      const std::string& aKey, const std::vector<TPL>& aValue,                 \
      bool expandable) const {                                                 \
...
// file thereby overwritting any file contents.
//
...
bool isValidVersionNr(const std::string& versionNr) {
  ...
  return (sscanf(versionNr.c_str(), "%d.%d.%d%10s", &release, &update, &patch,
                 toomuch) == 3);
}
...
std::string ParameterSetImpl::fullModuleName(
