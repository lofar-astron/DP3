...
void MultiDirSolver::init(size_t nAntennas, size_t nDirections,
...
void MultiDirSolver::makeSolutionsFinite1pol(
...
void MultiDirSolver::makeSolutionsFinite4pol(
    std::vector<std::vector<DComplex> >& solutions) {
  for (std::vector<DComplex>& solVector : solutions) {
...
bool MultiDirSolver::assignSolutions(
    std::vector<std::vector<DComplex> >& solutions,
    ...
    for (size_t i = 0; i != solutions[chBlock].size(); i += NPol) {
      ...
      if (NPol == 1) {
        if (solutions[chBlock][i] != 0.0) {
          double a =
              std::abs((nextSolutions[chBlock][i] - solutions[chBlock][i]) /
                       solutions[chBlock][i]);
          if (std::isfinite(a)) {
            avgAbsDiff += a;
            ++n;
...
MultiDirSolver::SolveResult MultiDirSolver::processScalar(
  ...
  do {
    ...
    hasConverged =
        assignSolutions<1>(solutions, nextSolutions, !constraintsSatisfied,
...
void MultiDirSolver::performScalarIteration(
    ...
    for (size_t baseline = 0; baseline != _ant1.size(); ++baseline) {
      ...
      if (antenna1 != antenna2) {
        ...
        for (size_t ch = channelIndexStart; ch != channelIndexEnd; ++ch) {
          const size_t dataIndex1 = ch - channelIndexStart +
                                    (timeIndex + antenna1 * nTimes) *
                                        curChannelBlockSize,
                       dataIndex2 = ch - channelIndexStart +
                                    (timeIndex + antenna2 * nTimes) *
                                        curChannelBlockSize;
          for (size_t p1 = 0; p1 != 4; ++p1) {
            ...
            for (size_t d = 0; d != _nDirections; ++d) {
              ...
              gTimesC2(dataIndex1 * 4 + p1, d) = std::conj(
                  solutions[solIndex1] * predicted);  // using a* b* = (ab)*
              gTimesC1(dataIndex2 * 4 + p2, d) =
  ...
  for (size_t ant = 0; ant != _nAntennas; ++ant) {
    ...
    if (success && x(0, 0) != 0.) {
      ...
      for (size_t d = 0; d != _nDirections; ++d)
        nextSolutions[ant * _nDirections + d] =
...
MultiDirSolver::SolveResult MultiDirSolver::processFullMatrix(
  ...
  do {
    ...
    double avgSquaredDiff;
    hasConverged =
        assignSolutions<4>(solutions, nextSolutions, !constraintsSatisfied,
                           avgSquaredDiff, step_magnitudes);
    if (statStream) {
...
bool MultiDirSolver::detectStall(
...
void MultiDirSolver::performFullMatrixIteration(
    size_t channelBlockIndex, std::vector<Matrix>& gTimesCs,
    std::vector<Matrix>& vs, const std::vector<DComplex>& solutions,
    std::vector<DComplex>& nextSolutions) {
  for (size_t ant = 0; ant != _nAntennas; ++ant) {
  ...
  for (size_t timeIndex = 0; timeIndex != nTimes; ++timeIndex) {
    ...
    for (size_t baseline = 0; baseline != _ant1.size(); ++baseline) {
      ...
      if (antenna1 != antenna2) {
        ...
        // dataIndex2.
        Matrix &gTimesC1 = gTimesCs[antenna1], &v1 = vs[antenna1],
               &gTimesC2 = gTimesCs[antenna2], &v2 = vs[antenna2];
        for (size_t d = 0; d != _nDirections; ++d)
          modelPtrs[d] =
              &_buffer.ModelData()[timeIndex][d]
                                  [(channelIndexStart + baseline * _nChannels) *
        ...
        const Complex* dataPtr =
            &_buffer.Data()[timeIndex]
                           [(channelIndexStart + baseline * _nChannels) * 4];
        for (size_t ch = channelIndexStart; ch != channelIndexEnd; ++ch) {
          const size_t dataIndex1 = 2 * (ch - channelIndexStart +
                                         (timeIndex + antenna1 * nTimes) *
                                             curChannelBlockSize),
                       dataIndex2 = 2 * (ch - channelIndexStart +
                                         (timeIndex + antenna2 * nTimes) *
                                             curChannelBlockSize);
    ...
    if (success && x(0, 0) != 0.) {
      for (size_t d = 0; d != _nDirections; ++d) {
        for (size_t p = 0; p != 4; ++p) {
          ...
          // and /)
          nextSolutions[(ant * _nDirections + d) * 4 + p] =
              std::conj(x(d * 2 + p % 2, p / 2));
        }
      }
    } else {
      for (size_t i = 0; i != _nDirections * 4; ++i) {
        nextSolutions[ant * _nDirections * 4 + i] =
            std::numeric_limits<double>::quiet_NaN();
      }
    }
  }
...
void MultiDirSolver::showTimings(std::ostream& os, double duration) const {
  ...
  // _timerSolve.Seconds()/duration << "% spent in solve" << std::endl; os << "
  // " << std::fixed << std::setprecision(2) <<
  // _timerFillMatrices.Seconds()/duration << "% spent in filling matrices" <<
  // std::endl;
  if (!_constraints.empty()) {
    // os << "                " << std::fixed << std::setprecision(2) <<
    // _timerConstrain.Seconds()/duration << "% spent in constraints" <<
    // std::endl;
