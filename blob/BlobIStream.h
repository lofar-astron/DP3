...
class BlobIStream {
 ...
 private:
  ...
  dp3::common::DataFormat itsDataFormat;
...
inline int64_t BlobIStream::tellPos() const { return itsStream->tellPos(); }

inline bool BlobIStream::mustConvert() const { return itsMustConvert; }

inline dp3::common::DataFormat BlobIStream::dataFormat() const {
...
inline BlobIStream& BlobIStream::operator>>(std::complex<T>& value) {
  ...
  if (itsMustConvert) {
    dp3::common::dataConvert(itsDataFormat, &value, 1);
  }
  return *this;
}
template <typename T>
inline void BlobIStream::get(std::complex<T>* values, uint64_t nrval) {
  ...
  if (itsMustConvert) {
    dp3::common::dataConvert(itsDataFormat, values, nrval);
  }
}
