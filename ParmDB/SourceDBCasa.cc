...
void SourceDBCasa::createTables(const string& tableName) {
  ...
  tdpat.comment() =
      String("Table containing the patches in the Local Sky Model");
...
void SourceDBCasa::addSource(const SourceInfo& sourceInfo,
  ...
  rownr_t patchId;
  {
    ...
    Table table =
        itsPatchTable(itsPatchTable.col("PATCHNAME") == String(patchName));
    if (table.nrow() != 1)
...
vector<PatchInfo> SourceDBCasa::getPatchInfo(int category,
  ...
  Vector<Double> ab(
      ROScalarColumn<double>(table, "APPARENT_BRIGHTNESS").getColumn());
...
SourceInfo SourceDBCasa::getSource(const string& sourceName) {
  TableLocker lockers(itsSourceTable, FileLocker::Read);
  Table table =
      itsSourceTable(itsSourceTable.col("SOURCENAME") == String(sourceName));
  if (table.nrow() == 0)
...
vector<SourceInfo> SourceDBCasa::readSources(const Table& table) {
  ...
  if (table.tableDesc().isColumn("SPINX_NTERMS")) {
    Vector<unsigned int> sd(
        ROScalarColumn<unsigned int>(table, "SPINX_NTERMS").getColumn());
    Vector<double> sr(
        ROScalarColumn<double>(table, "SPINX_REFFREQ").getColumn());
