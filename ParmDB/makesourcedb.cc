...
vector<string> string2vector(const string& value,
...
void readShapelet(const string& fileName, Array<double>& coeff, double& scale) {
...
void fillShapelet(SourceInfo& srcInfo, const string& shpI, const string& shpQ,
                  const string& shpU, const string& shpV) {
  double scaleI = 0;
  double scaleQ = 0;
  double scaleU = 0;
  double scaleV = 0;
...
void calcRMParam(double& polfrac, double& polang, double fluxi0, double fluxq,
                 double fluxu, const vector<double>& spinx, double rm,
...
void process(const string& line, SourceDB& pdb, const SdbFormat& sdbf,
             const string& prefix, const string& suffix, bool check,
  ...
  if (!(ra > -6.3 && ra < 6.3 && dec > -1.6 && dec < 1.6))
    throw std::runtime_error("RA " + to_string(ra) + " or DEC " +
                             to_string(dec) + " radians is outside boundaries");
  ...
  vector<double> spinx(vector2real(
      string2vector(values, sdbf.fieldNrs[SpInxNr], vector<string>()), 0.));
