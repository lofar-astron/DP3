...
ParmFacadeDistr::ParmFacadeDistr(const string& tableName)
    : itsPort(getPort()), itsConn(itsPort) {
  ...
  string command("startparmdbdistr " + itsPort + ' ' + cdescName + ' ' +
                 tableName);
  ...
  for (int i = 0; i < itsConn.size(); ++i) {
    ...
    if (i == 0) {
      ...
      if (defValues.size() != itsDefValues.size()) {
        LOG_WARN_STR("DEFAULTVALUES sizes of parts "
                     << itsPartNames[0] << " and " << itsPartNames[i]
                     << " differ");
      }
    }
    bbi.finish();
...
ParmFacadeDistr::~ParmFacadeDistr() { quit(); }
...
void ParmFacadeDistr::freePort() { theirFreePorts.push_back(itsPort); }
...
vector<string> ParmFacadeDistr::getDefNames(
...
void ParmFacadeDistr::addDefValues(const Record& values, bool check) {
  BlobString buf;
...
Record ParmFacadeDistr::getValues(const string& parmNamePattern, double freqv1,
  ...
  bbo.blobStream() << parmNamePattern << freqv1 << freqv2 << freqStep << timev1
                   << timev2 << timeStep << includeDefaults;
  bbo.finish();
  itsConn.writeAll(buf);
  // Read the replies.
...
Record ParmFacadeDistr::getValues(const string& parmNamePattern,
  ...
  bbo.blobStream() << parmNamePattern << freqv1 << freqv2 << timev1 << timev2
...
Record ParmFacadeDistr::getValuesGrid(const string& parmNamePattern,
  ...
  bbo.blobStream() << parmNamePattern << box.lowerX() << box.upperX()
...
Record ParmFacadeDistr::getCoeff(const string& parmNamePattern, double freqv1,
    ...
    if (checkStatus(bbi, i)) {
      ...
      if (!steps.empty()) {
        if (defSteps.empty()) {
          ...
          if (!ok) {
            LOG_WARN_STR("Default step values of part "
                         << itsPartNames[i] << " differ from the other parts");
          }
        }
      }
    }
...
void ParmFacadeDistr::deleteValues(const string& parmNamePattern, double freqv1,
                                   double freqv2, double timev1, double timev2,
                                   bool asStartEnd) {
  BlobString buf;
...
Record ParmFacadeDistr::combineRemote(const vector<Record>& recs) const {
  ...
  for (set<String>::const_iterator iter = names.begin(); iter != names.end();
       ++iter) {
...
void ParmFacadeDistr::findParmNames(const vector<Record>& recs,
  ...
  for (vector<Record>::const_iterator iter = recs.begin(); iter != recs.end();
       ++iter) {
...
void ParmFacadeDistr::combineInfo(const String& name,
  ...
  for (vector<Record>::const_iterator iter = recs.begin(); iter != recs.end();
    ...
    if (iter->isDefined(name)) {
      ...
      if (values.empty()) {
        ...
        ASSERT(allNear(tcenters, rec.asArrayDouble("times"), 1e-10));
      }
  ...
  for (unsigned int i = 1; i < indexf.size(); ++i) {
    ...
    if (arr.size() != lastUsed->size() || !allNear(arr, *lastUsed, 1e-10)) {
      index.push_back(inx);
