 ...
 */
std::vector<BDABuffer> CreateBDABuffers(std::complex<float> data_value,
                                        float weight_value) {
  std::vector<BDABuffer> buffers;
  ...
  for (std::size_t i = 0; i < 3; ++i) {
    BDABuffer buffer(kNBaselines * kNChannels * kNCorrelations);
    ...
    buffer.AddRow(time, kInterval, kInterval, 2, kNChannels / 2, kNCorrelations,
                  data.data(), flags, weights.data(), flags, nullptr);
...
BOOST_AUTO_TEST_CASE(assign_and_weight) {
  ...
  std::vector<BDABuffer> data = CreateBDABuffers(kDataValue, kWeight);
  std::vector<std::vector<BDABuffer>> model_data{
      CreateBDABuffers(kModelValue1, kModelWeight),
      CreateBDABuffers(kModelValue2, kModelWeight)};
  BDASolverBuffer buffer;
  buffer.AssignAndWeight(data, std::move(model_data));
