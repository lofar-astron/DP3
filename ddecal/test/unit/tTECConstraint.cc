...
BOOST_AUTO_TEST_CASE(test_tec_only_constraint) {
  TECConstraint constraint(TECConstraint::Mode::kTecOnly);
  ...
  Constraint::Result tec_result = constraint_result[0];
  Constraint::Result error_result = constraint_result[1];
  ...
  BOOST_CHECK_EQUAL_COLLECTIONS(tec_result.dims.begin(), tec_result.dims.end(),
  ...
  BOOST_CHECK_EQUAL_COLLECTIONS(error_result.dims.begin(),
                                error_result.dims.end(), expected_dims.begin(),
                                expected_dims.end());
...
BOOST_AUTO_TEST_CASE(test_tec_and_phase_constraint) {
  TECConstraint constraint(TECConstraint::Mode::kTecAndCommonScalar);
  ...
  Constraint::Result tec_result = constraint_result[0];
  Constraint::Result phase_result = constraint_result[1];
  Constraint::Result error_result = constraint_result[2];
  ...
  BOOST_CHECK_EQUAL_COLLECTIONS(phase_result.dims.begin(),
                                phase_result.dims.end(), expected_dims.begin(),
                                expected_dims.end());
  ...
  for (size_t ant = 1; ant < kNAntennas; ++ant) {
    ...
    BOOST_CHECK_SMALL(std::fmod(phase_result.vals[ant] -
