...
inline void TestIterativeDiagonal(SolverTester& solver_tester,
  ...
  const SolveData data(
      solver_buffer, SolverTester::kNChannelBlocks, SolverTester::kNAntennas,
      std::vector<size_t>(SolverTester::kNDirections, 1),
...
BOOST_FIXTURE_TEST_CASE(iterative_diagonal_cuda, SolverTester,
                        *boost::unit_test::label("slow")) {
  dp3::ddecal::IterativeDiagonalSolverCuda<aocommon::MC2x2F> solver;
  TestIterativeDiagonal(*this, solver);
}
BOOST_FIXTURE_TEST_CASE(iterative_diagonal_cuda_keep_buffers, SolverTester,
                        *boost::unit_test::label("slow")) {
  dp3::ddecal::IterativeDiagonalSolverCuda<aocommon::MC2x2F> solver{true};
  TestIterativeDiagonal(*this, solver);
}
...
BOOST_FIXTURE_TEST_CASE(iterative_full_jones, SolverTester,
  ...
  const dp3::ddecal::BdaSolverBuffer& solver_buffer = FillBDAData();
  dp3::ddecal::SolveData data(solver_buffer, kNChannelBlocks, kNAntennas,
                              std::vector<size_t>(kNDirections, 1), Antennas1(),
                              Antennas2(), false);
...
BOOST_FIXTURE_TEST_CASE(lbfgs_bounded_diagonal, SolverTester,
                        *boost::unit_test::label("slow")) {
  SetDiagonalSolutions(false);
  dp3::ddecal::LBFGSSolver solver(kRobustDOF, kBatchIterations * 2, kHistory,
...
BOOST_FIXTURE_TEST_CASE(lbfgs_bounded_scalar, SolverTester,
                        *boost::unit_test::label("slow")) {
  SetScalarSolutions(false);
  dp3::ddecal::LBFGSSolver solver(kRobustDOF, kBatchIterations, kHistory,
...
BOOST_FIXTURE_TEST_CASE(lbfgs_bounded_full_jones, SolverTester,
                        *boost::unit_test::label("slow")) {
  SetDiagonalSolutions(false);
  // Since we have more unknowns, reduce the minibatch size
  // to increase the constraints per-minibatch
