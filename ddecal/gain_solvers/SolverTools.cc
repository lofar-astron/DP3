...
void AssignAndWeight(
  ...
  schaapcommon::StaticFor<std::size_t> loop;
  loop.Run(0, n_times, [&](std::size_t time_start, std::size_t time_end) {
    for (std::size_t timestep = time_start; timestep != time_end; ++timestep) {
      ...
      // flagged. Although the initialization to false can be avoided by
      // directly assigning flags for the first correlation, a benchmark showed
      // that the (simpler) code below gives slightly better performance. Use
      // 'int' instead of 'bool' for flags since XSimd uses integers when or'ing
      // flags. Using integers directly prevents unpacking booleans to integers
      // beforehand and packing integers to booleans afterwards.
