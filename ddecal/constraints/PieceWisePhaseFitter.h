...
class PieceWisePhaseFitter {
 public:
   ...
   *
   * The data is divided into chunks with given size, and in each chunk a line
   * is fitted (offset & gradient). The fitted line of each chunk is written
   * into fittedData. The resulting line will have a 'jump' between each chunk.
   * @param nu Frequencies of the channels (in Hz).
   * @param data The values, same size as @c nu.
   * @param fittedData A vector of at least size @c nu.size(), in which the
   * fitted lines are stored.
   */
   ...
   *
   * A window of size @c chunkSize is slid over the data, and a line is fitted
   * at each position. The values near the left and right side of the border are
   * evaluated with the fit to the most left/right window. This is slower than
   * @ref PieceWiseFit(), but it produces a smoother & more accurate fit.
   * The fit is not necessarily entirely smooth, because the line fit tries to
   * guess the right wrapping of the samples. Since that is not a linear
   * process, this can still cause slight jumps.
   *
   * @param nu Frequencies of the channels (in Hz).
   * @param data The values, same size as @c nu.
   ...
   * sample x for which the sum of all values smaller than x is less than half
   * of the total weight, and idem for all values larger than x. For example,
   * given the sequence 1,2,3,4,5 with weights 1,100,1,1,1; the weighted median
   * is '2'. (sum of weight=104, values < 2 a have weight of 1, values > 2 have
   * a weight of 3). This function is internally used by the piece-wise phase
   * fitter.
 ...
 private:
  ...
  /**
   * No longer used -- I tried removing samples that deviated far from the
   * fitted line and refit, but this seemed not to help.
   */
