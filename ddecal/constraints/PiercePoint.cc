...
PiercePoint::PiercePoint(double height) : itsValue(3) {
...
void PiercePoint::init(const casacore::MPosition &ant,
  ...
  itsC = mPosition(0) * mPosition(0) + mPosition(1) * mPosition(1) +
         mPosition(2) * mPosition(2) -
         (itsIonoHeight + PiercePoint::EarthRadius) *
             (itsIonoHeight + PiercePoint::EarthRadius);
}
...
void PiercePoint::evaluate(casacore::MEpoch time) {
  ...
  const casacore::MDirection dir =
      casacore::MDirection::Convert(itsDirection, myref)();
