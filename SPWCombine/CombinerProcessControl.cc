...
tribool CombinerProcessControl::run() {
  try {
    ...
    int sourceMS = 0;
    for (unsigned int i = 0; i < itsInMS.size();
         i++)  // to search the shortest MS
    {
    ...
    if (sourceMS > 0) {
      cout << "Not all sources are the same lenght, using te shortest one."
           << endl;
    }

    Table TempTable =
        tableCommand(string("SELECT "
                            "UVW,FLAG_CATEGORY,WEIGHT,SIGMA,ANTENNA1,ANTENNA2,"
                            "ARRAY_ID,DATA_DESC_ID,") +
                     string("EXPOSURE,FEED1,FEED2,FIELD_ID,FLAG_ROW,INTERVAL,"
                            "OBSERVATION_ID,PROCESSOR_ID,") +
                     string("SCAN_NUMBER,STATE_ID,TIME,TIME_CENTROID,WEIGHT_"
                            "SPECTRUM,FLAG FROM ") +
    ...
    tableCommand(string("DELETE FROM ") + itsOutMS +
                 string("/DATA_DESCRIPTION WHERE rownumber() > 1"));
    tableCommand(string("DELETE FROM ") + itsOutMS +
                 string("/SPECTRAL_WINDOW WHERE rownumber() > 1"));
    ...
    // ugly workaround MSSpectral window does no allow deleting and then
    // recreating columns
...
tribool CombinerProcessControl::init() {
  try {
    ...
    cout << string(COMBINER_VERSION) +
                string(" spw combine by Adriaan Renting for LOFAR CS1\n") +
                string(
                    "This is experimental software, please report errors or "
                    "requests to renting@astron.nl\n") +
                string(
                    "Documentation can be found at: "
                    "www.lofar.org/operations/"
                    "doku.php?id=engineering:software:postprocessing_"
                    "software\n");
...
//<<<===============================
tribool CombinerProcessControl::pause(const std::string&) { return false; }

//===============>>> CombinerProcessControl::quit
//<<<================================
tribool CombinerProcessControl::quit() { return true; }

//===============>>> CombinerProcessControl::release
//<<<=============================
tribool CombinerProcessControl::release() { return false; }
//===============>>> CombinerProcessControl::recover
//<<<=============================
tribool CombinerProcessControl::recover(const std::string&) { return false; }

//===============>>> CombinerProcessControl::reinit
//<<<==============================
tribool CombinerProcessControl::reinit(const std::string&) { return false; }
