...
void Axis::throwNotFound(double x) const {
  throw std::runtime_error("Axis::locate: cell " + std::to_string(x) +
                           " not found");
}
  ...
  for (int i = 0; i < nr; ++i) {
    ...
    if (!((low1 == low2 || casacore::near(low1, low2)) &&
      ...
      throw std::runtime_error(
          "Axis::combine: interval [" + std::to_string(low1) + ',' +
          std::to_string(upp1) + "] mismatches [" + std::to_string(low2) + ',' +
          std::to_string(upp2) + ']');
  }
...
RegularAxis::RegularAxis() : itsStart(-1e30), itsWidth(2e30), itsCount(1) {
  setup(itsStart, itsWidth, itsCount);
}

RegularAxis::RegularAxis(double start, double width, unsigned int count,
                         bool asStartEnd)
    : itsStart(start), itsWidth(width), itsCount(count) {
  if (asStartEnd) {
...
OrderedAxis::OrderedAxis(const std::vector<double>& starts,
                         const std::vector<double>& ends, bool asStartEnd) {
...
Axis::ShPtr OrderedAxis::compress(size_t factor) const {
  assert(factor > 0);
  size_t count =
      static_cast<size_t>(std::ceil(static_cast<double>(size()) / factor));
