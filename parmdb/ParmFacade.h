...
class ParmFacade {
 public:
  ...
  std::map<std::string, std::vector<double>> getValuesMap(
    ...
    return getValuesMap(parmNamePattern, freqv1, freqv2, 0, timev1, timev2,
  ...
  casacore::Record getValues(const string& parmNamePattern,
                             ...
                             bool asStartEnd = true,
                             bool includeDefaults = false) {
    return itsRep->getValues(parmNamePattern, freqv1, freqv2, timev1, timev2,
                             asStartEnd, includeDefaults);
  }
  ...
  casacore::Record getValuesGrid(const string& parmNamePattern,
                                 ...
                                 double timev1 = -1e30, double timev2 = 1e30,
                                 bool asStartEnd = true) {
    return itsRep->getValuesGrid(parmNamePattern, freqv1, freqv2, timev1,
                                 timev2, asStartEnd);
  }
  ...
  casacore::Record getCoeff(const string& parmNamePattern,
                            ...
                            double timev1 = -1e30, double timev2 = 1e30,
                            bool asStartEnd = true) {
    return itsRep->getCoeff(parmNamePattern, freqv1, freqv2, timev1, timev2,
                            asStartEnd);
  }
  ...
  ///@{
  void lock(bool lockForWrite) { itsRep->lock(lockForWrite); }
