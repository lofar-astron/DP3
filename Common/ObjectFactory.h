...
template <typename Signature, typename TypeId>
class ObjectFactory;
/// @}
}  // namespace DP3
...
class ObjectFactory<Base*(BOOST_PP_ENUM_PARAMS(n, A)), TypeId> {
 ...
 public:
  ...
  bool registerClass(TypeId id) {
    return itsCreatorMap
        .insert(value_type(id, static_cast<CreatorFunc>(&doCreate<Derived>)))
        .second;
  }
  ...
  bool unregisterClass(TypeId id) { return (itsCreatorMap.erase(id)); }
  ...
  const_iterator begin() const { return itsCreatorMap.begin(); }

  /// Return a \c const iterator for the position after the last element in
  /// \c itsCreatorMap.
  const_iterator end() const { return itsCreatorMap.end(); }
