...
public:
	...
	/// before all keys in the subset.
        std::shared_ptr<ParameterSetImpl> makeSubset(const std::string& baseKey,
...
private:
	...
	/// \c readStream method.
	/// @{
	void	readFile   (const std::string& theFile, const std::string& prefix, const bool merge);
	void	readBuffer (const std::string& theFile, const std::string& prefix, const bool merge);
	void	readStream (std::istream&	inputStream, const std::string& prefix, const bool merge);
	/// @}

	/// Find the key \p aKey. If \p doThrow \c == \c true (the default) an exception
	/// is thrown when \p aKey is not found. Otherwise, end() is returned.
