...
using dp3::sky_model::PatchSumInfo;
using dp3::sky_model::SourceInfo;
...
void addValue(std::map<std::string, dp3::parmdb::ParmValue>& fieldValues,
              const std::string& name, double value) {
  fieldValues[name] = ParmValue(value);
}

void add(std::map<std::string, dp3::parmdb::ParmValue>& fieldValues,
         FieldNr field, double value) {
  fieldValues[theFieldNames[field]] = ParmValue(value);
}

void addSpectralIndices(
    std::map<std::string, dp3::parmdb::ParmValue>& fieldValues,
    const std::vector<double>& spectralIndices, double referenceFrequency) {
  // Spectral index terms are typically of order 1. An absolute value above
  // 1e7 is thus clearly incorrect. Note that polynomial spectral indices
  // as generated by WSClean can be significantly larger than order 1.
  const double kMaxSpectralIndex = 1.0e7;
...
void process(const std::string& line, dp3::sky_model::SkyModel& pdb,
  ...
  std::map<std::string, ParmValue> fieldValues;
  std::vector<std::string> values;
  // Process the line.
...
static void ParseSkyModel(dp3::sky_model::SkyModel& pdb, std::istream& input,
                          const SdbFormat& sdbf, const std::string& prefix,
...
SkyModel ReadSkyModel(const std::string& filename, const std::string& format) {
  if (!HasSkyModelExtension(filename)) {
    throw std::runtime_error(
        "Extension of sky-model file '" + filename +
        " is unknown -- format not supported. Should be .txt or .sky_model. "
