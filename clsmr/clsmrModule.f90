!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
! File zlsmrModule.f90
!
!     zLSMR
!
! zLSMR   solves complex Ax = b or min ||Ax - b|| with or without damping,
...
module clsmrModule
  ...
  use  clsmrDataModule,    only : sp, ip, zero, one, zzero, zone
  use  clsmrblasInterface, only : scnrm2, sscal
  implicit none
  private
  public   :: cLSMR
...
contains
  ...
  subroutine cLSMR  ( m, n, Aprod1, Aprod2, b, damp,                    &
    ...
    complex(sp), intent(in)      :: b(m)
    complex(sp), intent(out)     :: x(n)
    real(sp), intent(in)         :: atol, btol, conlim, damp
    real(sp), intent(out)        :: normA, condA, normr, normAr, normx
    ...
    !
    ! NOTE: If A is symmetric (Hermitian), LSMR should not be used!
    ...
    if (beta > zero) then
       call sscal (m, cmplx(one/beta,0,sp), u, 1)
       call Aprod2(m, n, v, u)          ! v = A'*u
       alpha = scnrm2 (n, v, 1)
    end if

    if (alpha > zero) then
       call sscal (n, cmplx(one/alpha,0,sp), v, 1)
       w = v
    end if
    ...
    do
       ...
       !    alpha*v = A'*u -  beta*v.
       !----------------------------------------------------------------
       call sscal (m,cmplx(- alpha,0,sp), u, 1)
       ...
       if (beta > zero) then
          ...
          call sscal (n, cmplx(- beta,0,sp), v, 1)
       ...
       if (show) then
          ...
          if (prnt) then        ! Print a line for this iteration
             ...
             pcount = pcount + 1
             write(nout,1500) itn,abs(x(1)),normr,normAr,test1,test2,normA,condA
          end if
       end if
...
800 if (show) then                      ! Print the stopping condition.
 ...
 1200 format(/ "   Itn   abs(x(1))           norm r         A'r   ", &
      ' Compatible    LS      norm A    cond A')
 1300 format(/ "   Itn   abs(x(1))          norm rbar    Abar'rbar", &
 ...
 3000 format(a, 5x, a)
  ...
  end subroutine cLSMR

  !+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

end module cLSMRmodule
